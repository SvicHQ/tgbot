...
@bot.on_message(filters.command(["unmute", "sunmute"], ["/", "!", "-", "."]))
@pm_error
async def func_unmute(_, message: Message):
    ...
    if not re_msg:
        return await message.reply_text(
            "I don't know who you are talking about! Mention or Reply the member whom you want to unmute!\n"
    ...
    if not is_silent:
        await message.reply_text(f"{victim.mention} has been unmuted." + (f"\nReason: {reason}" if reason else ""))
