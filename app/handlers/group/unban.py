...
@bot.on_message(filters.command(["unban", "sunban"], ["/", "!", "-", "."]))
@pm_error
async def func_unban(_, message: Message):
    chat = message.chat
    user = message.from_user or message.sender_chat
    username_reason = extract_cmd_args(message.text, message.command)
    re_msg = message.reply_to_message
    mad_quote = "Huh! Don't spend too many brain cells. Overthinking is just as bad as underthinking."
    ...
    if not victim:
        return await message.reply_text(
            "I don't know who you are talking about! Mention or Reply the member whom you want to unban!\n"
            f"E.g`/{message.command[0]} @username reason`"
        )
    
    if victim.id == bot.me.id:
        return await message.reply_text(mad_quote)
    ...
    if admin_roles["user_admin"] and not admin_roles["user_admin"].privileges.can_restrict_members:
        return await message.reply_text("You don't have enough permission to unrestrict chat members!")
    ...
    if not is_silent:
        await message.reply_text(f"{victim.mention} has been unbanned." + (f"\nReason: {reason}" if reason else ""))
