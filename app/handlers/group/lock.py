...
@bot.on_message(filters.command(["lock", "slock"], ["/", "!", "-", "."]))
@pm_error
async def func_lock(_, message: Message):
    ...
    if admin_roles["user_admin"] and not admin_roles["user_admin"].privileges.can_change_info:
        return await message.reply_text("You don't have enough permission to lock this chat!")
    ...
    if not admin_roles["bot_admin"].privileges.can_change_info:
        return await message.reply_text("I don't have enough permission to lock this chat!")
