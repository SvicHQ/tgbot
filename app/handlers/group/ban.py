...
@bot.on_message(filters.command(["ban", "dban", "sban"], ["/", "!", "-", "."]))
@pm_error
async def func_ban(_, message: Message):
    chat = message.chat
    ...
    if victim.id == bot.me.id:
        return await message.reply_text("I'm not going to ban myself!")
    ...
    if admin_roles["victim_admin"] or admin_roles["victim_owner"]:
        return await message.reply_text("I'm not going to ban an admin! You must be kidding!")
    ...
    if not is_silent:
        await message.reply_text(f"{victim.mention} has been banned from this chat." + (f"\nReason: {reason}" if reason else ""))
