...
@bot.on_message(filters.command(["demote", "sdemote"], ["/", "!", "-", "."]))
@pm_error
async def func_demote(_, message: Message):
    ...
    if not victim:
        return await message.reply_text(
            "I don't know who you are talking about! Mention or Reply the member whom you want to demote!\n"
            f"E.g`/{message.command[0]} @username reason`"
        )
    
    if victim.id == bot.me.id:
        return await message.reply_text("I'm not going to demote myself!")
    ...
    if not is_silent:
        await message.reply_text(f"{victim.mention} has been demoted." + (f"\nReason: {reason}" if reason else ""))
