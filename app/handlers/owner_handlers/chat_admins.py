...
async def func_cadmins(_, message: Message):
    ...
    if not chat_id:
        return await message.reply_text(f"`/{message.command[0]} ChatID or Username` to get specified chat admin list.\n<i>Note: only works if this bot is in that chat!</i>")
    ...
    try:
        async for admin in bot.get_chat_members(chat_id=int(chat_id), filter=ChatMembersFilter.ADMINISTRATORS):
            formatted_msg = f"• {admin.user.mention} - <i>{admin.custom_title or '~'}</i>\n"
