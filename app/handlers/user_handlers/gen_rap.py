...
async def func_rap(_, message: Message):
    ...
    # hex_data = extract_cmd_args(message.text, message.command)

    # if not hex_data:
    #     return await message.reply_text(
    #         f"Use `/{message.command[0]} rapData (hex)`\n"
    #         f"E.g. `/{message.command[0]} D78710F4C0979FAD9CDB40C612C94F60`\n"
    ...
    #     return await message.reply_text(response[result])
